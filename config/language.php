<?php

_('txt-program-name-label');
_('txt-program-name-help-block');
_('txt-program-number-label');
_('txt-program-label-help-block');
_('txt-program-invoice-method-label');
_('txt-program-invoice-method-help-block');
_('txt-call-session-session-label');
_('txt-call-session-session-placeholder');
_('txt-call-session-quota-help-block');
_('txt-call-session-quota-label');
_('txt-call-session-open-for-registration-label');
_('txt-call-session-open-for-registration-help-block');
_('txt-call-session-idea-tool-label');
_('txt-idea-tool-help-block');
_('txt-call-session-date-from-label');
_('txt-call-session-date-from-help-block');
_('txt-call-session-date-end-label');
_('txt-call-session-date-end-help-block');
_('txt-not-open-for-registration');
_('txt-open-for-registration');
_('txt-date-national-application-label');
_('txt-date-national-application-help-block');
_('txt-date-expected-funding-decision-label');
_('txt-date-expected-funding-decision-help-block');
_('txt-program-call');
_('txt-po-open-date');
_('txt-po-close-date');
_('txt-loi-submission-date-label');
_('txt-loi-submission-help-block');
_('txt-fpp-open-date');
_('txt-fpp-close-date');
_('txt-doa-submission-date-label');
_('txt-doa-submission-help-block');
_('txt-label-announcement-date-label');
_('txt-label-announcement-help-block');
_('txt-doa-requirements');
_('txt-doa-requirements-inline-help');
_('txt-loi-requirements');
_('txt-loi-requirements-inline-help');
_('txt-call-project-report-label');
_('txt-call-project-report-help-block');
_('txt-nda-requirements');
_('txt-nda-requirements-inline-help');
_('txt-program-call-active-label');
_('txt-program-call-active-inline-help');
_('txt-program');
_('txt-inactive-for-projects');
_('txt-active-for-projects');
_('txt-no-doa-required');
_('txt-doa-per-program-required');
_('txt-doa-per-project-required');
_('txt-doa-per-project-or-membership-required');
_('txt-no-nda-required');
_('txt-nda-per-call-required');
_('txt-nda-per-project-required');
_('txt-no-loi-required');
_('txt-loi-required');
_('txt-project-report-single');
_('txt-project-report-double');
_('txt-nda-file');
_('txt-contact');
_('txt-country');
_('txt-funder-country-help-block');
_('txt-funder-info-office-label');
_('txt-funder-info-office-placeholder');
_('txt-funder-info-office-help-block');
_('txt-funder-info-public-label');
_('txt-funder-info-public-placeholder');
_('txt-funder-info-public-help-block');
_('txt-show-on-website');
_('txt-funder-show-on-website-help-block');
_('txt-funder-sorting-position-label');
_('txt-funder-sorting-position-help-block');
_('txt-funder-website-label');
_('txt-funder-website-placeholder');
_('txt-funder-website-help-block');
_('txt-hide-on-website');
